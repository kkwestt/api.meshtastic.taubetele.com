exports.encodeServiceEnvelope = function (message) {
  const bb = popByteBuffer()
  _encodeServiceEnvelope(message, bb)
  return toUint8Array(bb)
}

function _encodeServiceEnvelope (message, bb) {
  // optional MeshPacket packet = 1;
  const $packet = message.packet
  if ($packet !== undefined) {
    writeVarint32(bb, 10)
    const nested = popByteBuffer()
    _encodeMeshPacket($packet, nested)
  ...
  if ($channel_id !== undefined) {
    writeVarint32(bb, 18)
    writeString(bb, $channel_id)
  }

  // optional string gateway_id = 3;
  const $gateway_id = message.gateway_id
  if ($gateway_id !== undefined) {
    writeVarint32(bb, 26)
    writeString(bb, $gateway_id)
  }
};

exports.decodeServiceEnvelope = function (binary) {
  return _decodeServiceEnvelope(wrapByteBuffer(binary))
}

function _decodeServiceEnvelope (bb) {
  const message = {}
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      case 1: {
        ...
        message.packet = _decodeMeshPacket(bb)
        bb.limit = limit
        break
      }
      ...
      // optional string channel_id = 2;
      case 2: {
        message.channel_id = readString(bb, readVarint32(bb))
        break
      }

      // optional string gateway_id = 3;
      case 3: {
        message.gateway_id = readString(bb, readVarint32(bb))
        break
      }
...
function readString (bb, count) {
  ...
  for (let i = 0; i < count; i++) {
    const c1 = bytes[i + offset]; var c2; var c3; var c4; var c
